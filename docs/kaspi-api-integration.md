...
#### Две цены, а не «цена и старая цена»
...
она действующая, скидка считается от неё. Поэтому в модели два поля —
`price` (основная) и `discountPrice` (со скидкой, может отсутствовать), а не
`price` и `oldPrice`.
